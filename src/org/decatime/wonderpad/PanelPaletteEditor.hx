...
class PanelPaletteEditor extends BoxLayout implements IObserver {
	...
	public function handleEvent(name:String, sender:IObservable, data:Dynamic) {
		if (name == DrawingSurface.EVT_STYLE_CHANGED) {
			...
			var element:IVisualElement = pnColor.getGridColor().getCellByColor(sty.getFillProperty().getColor());
			if (element != null) {
				pnColor.getGridColor().setSelected(element);
			}

			// updating the alpha value for this style
			pnColor.getAlphaSlider().setValue(sty.getFillProperty().getTransparency());
		}
	}
